...
function appointments(){
  ...
  return `<div class="top"><div><h1 class="page-title">Lista de agendamentos</h1><p class="page-sub">Visualize, edite ou exclua registros.</p></div><button class="btn btn-primary" data-nav="new">+ Novo agendamento</button></div><div class="card table-wrap"><table><thead><tr><th>DATA</th><th>HORA</th><th>Paciente</th><th>Hospital</th><th>Médico</th><th>Procedimento</th><th>Status</th><th>Ações</th></tr></thead><tbody>${state.appointments.map(a=>`<tr><td>${a.data_agendamento}</td><td>${formatTime(a.horario)}</td><td>${a.paciente}</td><td>${getName(state.lookups.hospitais,a.hospital_id,'hospital_id','nome_hospital')}</td><td>${getName(state.lookups.medicos,a.medico_id,'medico_id','nome_medico')}</td><td>${getName(state.lookups.procedimentos,a.procedimento_id,'procedimento_id','nome_procedimento')}</td><td><span class="badge ${statusClass(a.status_id)}">${getName(state.lookups.status,a.status_id,'status_id','nome_status')}</span></td><td><div class="actions"><button class="btn" data-edit="${a.agendamento_id}">✏️</button><button class="btn btn-danger" data-del="${a.agendamento_id}">🗑️</button></div></td></tr>`).join('')}</tbody></table></div>`}
...
function bindBlocked(){document.getElementById('blockForm').onsubmit=async e=>{e.preventDefault(); const p=Object.fromEntries(new FormData(e.target)); p.org_id=state.user.org_id;p.unidade_id=state.unit.unidade_id;p.criado_por_user_id=state.user.user_id; const convertDate=(dateStr)=>{const parts=dateStr.split('-');return parts[2]+'/'+parts[1]+'/'+parts[0];};p.data_inicio=convertDate(p.data_inicio);p.data_fim=convertDate(p.data_fim); try{await api('saveBlockedDate',p);await loadBase();renderShell('blocked');toast('Bloqueio cadastrado','success');}catch(err){toast(err.message,'danger')}}}
